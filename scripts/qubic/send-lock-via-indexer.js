/**
 * QSB Lock — sends a Lock transaction via Bob Node's /broadcastTransaction,
 * which captures the Lock event in its DB AND forwards to Core Lite.
 *
 * Usage:
 *   node scripts/qubic/send-lock-via-indexer.js [amount] [relayerFee] [toAddress] [seed]
 *
 * Defaults:
 *   amount     = 100000
 *   relayerFee = 1000
...
while (Date.now() < deadline) {
  ...
  if (currentTick >= targetTick) {
    ...
    try {
      const txRes = await fetch(`${bobUrl}/tx/${txId.toLowerCase()}`);
      if (txRes.ok) {
      ...
      console.log(`\nCould not reach Bob Node for tx status.`);
    }
    break;
  }
